...
void vcRender_OpaquePass(vcState *pProgramState, vcRenderContext *pRenderContext, vcRenderData &renderData)
{
  ...
  for (int y = 0; y < 300; ++y)
  {
    for (int x = 0; x < 100; ++x)
    {
      udDouble4x4 world = udDouble4x4::translation(
        -3968728.208223f + x * 2.5f,
        4340782.971250f + y * 2.5f,
        -2477439.792986f + 0.0f);// z * 2.5f);// (c / 1000) * 3.0f, (c % 1000) * 3.0f, 0.0f);
      ...
      vcRenderPolyInstance *pInstance = renderData.polyModels.PushBack();
      pInstance->pModel = pTestInstanceModel;
      pInstance->worldMat = world;
      pInstance->pSceneItem = nullptr;
      pInstance->pDiffuseOverride = pProgramState->pWhiteTexture;
      pInstance->tint = udFloat4::create(float(rand()) / RAND_MAX, float(rand()) / RAND_MAX, float(rand()) / RAND_MAX, 1.0f);
  ...
  // Polygon Models
  {
...
#if 1
    // remember: this will only work if we separate viewProjection into different constant buffer...which introduces wobbling
    static vcInstancedRenderer *pRenderer = nullptr;
    if (pRenderer == nullptr)
    {
      pRenderer = udAllocType(vcInstancedRenderer, 1, udAF_Zero);
      pRenderer->queues.Init(256 << 4);
      ...
      //      data.colour = udFloat4::create(float(rand()) / RAND_MAX, float(rand()) / RAND_MAX, float(rand()) / RAND_MAX, 1.0f);
      //      data.objectInfo = udFloat4::create(0, 0, 0, 0);
      //      pRenderer->queues.PushInstance(pTestInstanceModel, &data);
      //    }
      //  }
      //}
...
#if 1
    pRenderer->queues.Empty();
    uint64_t timer = udPerfCounterStart();

    static double timerd = 0.0;
    timerd += pProgramState->deltaTime;
    ...
    //for (int z = 0; z < 100; ++z)
    {
      for (int y = 0; y < 50; ++y)
      {
        ...
        {
          const int count = 600;
          vcInstanceGeometryData data[count] = {};
          for (int c = 0; c < count; ++c)
          {
            udDouble4x4 world = udDouble4x4::scaleUniform(1.0f,
              udDouble3::create(-3968728.208223f + (x + c) * 2.5f,
                4340782.971250f + y * 2.5f + timerd * 100.0f,
              -2477439.792986f));

            data[c].worldViewProjectionMatrix = udFloat4x4::create(pProgramState->pActiveViewport->camera.matrices.viewProjection * world);
            data[c].colour = udFloat4::create(x / 15.0f, y / 50.0f, 0.0f, 1.0f);// create(float(rand()) / RAND_MAX, float(rand()) / RAND_MAX, float(rand()) / RAND_MAX, 1.0f);
            data[c].objectInfo = udFloat4::create(0, 0, 0, 0);
          }

          pRenderer->queues.PushInstances(gInternalModels[vcInternalModelType_Cube], data, count);
        }
      }
    }
    

    float timeMS = udPerfCounterMilliseconds(timer);
    printf("%f\n", timeMS);
#endif

    renderData.instancedModels2.PushBack(pRenderer);
#endif

#if 0
    //TODO: submit pinrenderer
    //TODO: submit instanced lists
    for (int y = 0; y < 40; ++y)
    {
...
void vcRender_RenderScene(vcState *pProgramState, vcRenderContext *pRenderContext, vcRenderData &renderData, vcFramebuffer *pDefaultFramebuffer)
{
  ...
  if (pProgramState->settings.activeViewportCount > 1 && pProgramState->settings.camera.mapMode[pProgramState->activeViewportIndex] && pProgramState->settings.presentation.showCameraFrustumInMapMode)
